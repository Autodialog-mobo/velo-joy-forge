...
const T = {
  ...
  fr: {
    ...
    why1: <><strong>Votre client reste votre client.</strong> Le Frame-ID lie votre client à votre magasin : il revient pour l'entretien, les réparations et son prochain vélo. Vous ne gagnez pas uniquement sur le Frame-ID lui-même. Vous gagnez sur le client qui reste. Chaque Frame-ID remis est un client fidélisé.</>,
    ...
    rollout1: (price: string) => <>Aujourd'hui, vous remettez des Frame-ID à l'unité au même prix d'achat de <strong>{price} HTVA</strong> par Frame-ID.</>,
    rollout2: "Les packs préconditionnés (1 / 2 / 5, prêts pour le rayon avec code-barres article scannable dans votre système de caisse) se déploient progressivement, en commençant par un premier groupe de magasins. Une fois la phase pilote réussie, la commande et la livraison des packs seront élargies.",
  ...
  de: {
    ...
    why1: <><strong>Ihr Kunde bleibt Ihr Kunde.</strong> Die Frame-ID bindet den Kunden an Ihr Geschäft: er kommt zurück für Wartung, Reparatur und sein nächstes Fahrrad. Sie verdienen nicht nur an der Frame-ID selbst. Sie verdienen am Kunden, der bleibt. Jede Frame-ID, die Sie mitgeben, ist ein Kunde, den Sie halten.</>,
    ...
    rollout1: (price: string) => <>Heute geben Sie einzelne Frame-ID mit, zum exakt gleichen Einkaufspreis von <strong>{price} netto</strong> pro Frame-ID.</>,
    rollout2: "Die vorverpackten Bundles (1 / 2 / 5, regalfertig mit scannbarer Artikel-Barcode in deinem Kassasystem) rollen wir schrittweise aus, beginnend mit einer ersten Gruppe von Händlern. Sobald die Testphase erfolgreich abgeschlossen ist, wird die Bestellung und Lieferung der Bundles breiter verfügbar.",
  ...
  en: {
    ...
    rollout1: (price: string) => <>Today you hand out loose Frame-IDs, at the exact same purchase price of <strong>{price} excl. VAT</strong> per Frame-ID.</>,
    rollout2: "The pre-packaged bundles (1 / 2 / 5, ready as shelf items with scannable product barcode in your POS system) roll out step by step, starting with a first group of shops. Once the pilot phase is successfully completed, ordering and delivery of the bundles becomes more broadly available.",
  ...
  es: {
    ...
    rollout1: (price: string) => <>Hoy entregas Frame-IDs sueltos, al mismo precio de compra de <strong>{price} sin IVA</strong> por Frame-ID.</>,
    rollout2: "Los packs preempaquetados (1 / 2 / 5, listos como artículo de lineal con código de barras escaneable en tu sistema de caja) se despliegan paso a paso, empezando por un primer grupo de tiendas. Una vez completada con éxito la fase piloto, el pedido y entrega de los packs estará más ampliamente disponible.",
